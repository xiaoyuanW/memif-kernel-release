...
#endif
struct page *vm_normal_page(struct vm_area_struct *vma, unsigned long addr,				pte_t pte) { unsigned long pfn = pte_pfn(pte);
...
/* xzl -- modeled after @follow_page_mask.
 * @ppage: caller-provided array of page*
 *
 * XXX we don't deal with:
 *      - huge page
 *      - FOLL_NUMA, FOLL_MLOCK
 *      - migration entry
 *
 * return: the # of successfully followed pages.
 */
#if 0
...
EXPORT_SYMBOL(follow_page_range);
#endif
		...
		unsigned int flags)
{
	...
	printk("enter follow_page_ranage!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");

new_pgd:
	//pgd
	//base = __va(read_cr3() & PHYSICAL_PAGE_MASK); 
...
new_pmd:
	...
	ptep = pte_offset_kernel(pmd, address); //510
	...
	while(1){
		...
		if(!pte_present(pte)){
			printk("!pte_present: address %08lx ptep %lx\n", address, ptep);
			ppage[n] = NULL;
			return -1;
		}
		...
		if (unlikely(!ppage[n])) { /*page does not exist */
			printk("ops... page does not exit!\n");
		}
...
#if 0
// hym: for 2M page
// ref: mm/hugetlb.c hugetlb_fault()
// ref: mm/hugetlb.c follow_hugetlb_page()
int follow_huge_page_range(struct vm_area_struct *vma,
			unsigned long address, int count, struct page **ppage,
                 	unsigned int flags)
{
	struct mm_struct *mm = vma->vm_mm;
...
EXPORT_SYMBOL(follow_huge_page_range);
#endif
...
new_pud:
	...
	while(1){
		...
		ppage[n] = pte_page(pte);

		if(unlikely(!ppage[n])){
			printk("ops... page does not exit!\n");
			BUG_ON(unlikely(!ppage[n]));
		}
		// should add FOLL_GET, FOLL_TOUCH...later
