			   ...
			   dma_addr_t dma_src, size_t len, unsigned long flags)
{
	...
	i = 0;
	//printk("num_descs = %d\n", num_descs);
	//printk("xfercap_log = %d\n", 1 << ioat_chan->xfercap_log);
	do {
		size_t copy = min_t(size_t, len, 1 << ioat_chan->xfercap_log);

		//hym: what's the size of each descriptor???
		//printk("&&&&&&&& xfercap_log = %d\n", 1 << ioat_chan->xfercap_log);
		// 1 << ioat_chan->xfercap_log = 1048576
...
//hym: implement scatter/gather
struct dma_async_tx_descriptor *
ioat_dma_prep_sg(struct dma_chan *chan, struct scatterlist *dst_sg, 
                         unsigned int dst_nents, struct scatterlist *src_sg,
                         unsigned int src_nents, unsigned long flags)
{
	struct ioatdma_chan 		*ioat_chan = to_ioat_chan(chan);
	int 				i, idx, j, k, num_descs, total_num_descs;
	struct ioat_dma_descriptor 	*hw = NULL;
	struct ioat_ring_ent 		*desc = NULL;
	struct scatterlist 		*sgd, *sgs;
	unsigned long 			total_len = 0;

	if (test_bit(IOAT_CHAN_DOWN, &ioat_chan->state))
		return NULL;

	/*here, should check the length, which cannot larger than the max 
	number(or available number) of descriptors of each channel.
	*
	*    add code later........
	**/
	if(dst_nents > 256){ // 256 descriptors in each channel
		printk("err: descriptor number > 256 (max)\n");
		return NULL;
	}
	BUG_ON(dst_nents > 256);

	if(dst_nents != src_nents){
		printk("error: dst_nents != src_nents!\n");
		return NULL;
	}
	BUG_ON(dst_nents != src_nents);

	total_num_descs = 0;
	for(i= 0, sgd = dst_sg, sgs = src_sg;
			i < src_nents;
			i++, sgd = sg_next(sgd), sgs = sg_next(sgs)){

		unsigned long len = sg_dma_len(sgs);
		total_num_descs += ioat_xferlen_to_descs(ioat_chan, len);
	}

	if(likely(total_num_descs) &&
		ioat_check_space_lock(ioat_chan, total_num_descs) == 0){
		idx = ioat_chan->head;
	}else{
		return NULL;
	}
	...
	for(i = 0, sgd = dst_sg, sgs = src_sg; 
		...
		// how many descs are needed in each entry of sg 
		num_descs = sg_dma_len(sgs) / (1 << ioat_chan->xfercap_log); //xfercap_log: 1MB
		if((sg_dma_len(sgs) % (1 << ioat_chan->xfercap_log)) != 0){
			num_descs = num_descs + 1;	
		}

		for(k = 0; k < num_descs; k++, j++){

			//printk("%d:%s: sg[%d]'s %d desc\n", __LINE__, __func__,i,k);

			desc = ioat_get_ring_ent(ioat_chan, idx + j);
			BUG_ON(desc == NULL); // there is no enough desc

			hw = desc->hw;

			if((sg_dma_len(sgs) - (k * (1 << ioat_chan->xfercap_log))) < 
				(1 << ioat_chan->xfercap_log)){
				hw->size = sg_dma_len(sgs) - (k * (1 << ioat_chan->xfercap_log));
			}else{
				hw->size = 1 << ioat_chan->xfercap_log;
			}

			//printk("hw->size = %d\n", hw->size);
			total_len += hw->size;

			/* each ioat channel has 256 descriptors
			 * each descriptor can transfter 1M data at most
			 * so the largest data size that each channel can transfer is 256 * 1024 * 1024
			 */
			BUG_ON(total_len > 256 * 1024 * 1024);

			hw->ctl = 0;
			hw->src_addr = sg_dma_address(sgs) + (k * (1 << ioat_chan->xfercap_log));
			hw->dst_addr = sg_dma_address(sgd) + (k * (1 << ioat_chan->xfercap_log));
	...
	desc->txd.flags = flags;
	desc->len = total_len; //total size, bad... should change
	//printk("total_len = %d\n", total_len); //5.17
	hw->ctl_f.int_en = !!(flags & DMA_PREP_INTERRUPT);
	hw->ctl_f.fence = !!(flags & DMA_PREP_FENCE);
	hw->ctl_f.compl_write = 1;
...
*/			
			...
			len -= copy;
			hw->src_addr = sg_dma_address(sgs) + copy;
			hw->dst_addr = sg_dma_address(sgd) + copy;
