                         ...
                         unsigned int src_nents, unsigned long flags)
{
	...
	**/
	...
	if(likely(src_nents) &&
		ioat_check_space_lock(ioat_chan, src_nents) == 0){
		idx = ioat_chan->head;
	}else{
		return NULL;
	}
	...
	for(i = 0, sgd = dst_sg, sgs = src_sg; 
		...
		//hw->size = 4096; // page size is 4k, bad, should modify this...
		hw->size = sg_dma_len(sgs); 
		//total_len += sg_dma_len(sgs);
		total_len += sgs->dma_length;

		//printk("sgs_dma_len(sgs) = %d\n",sg_dma_len(sgs));
		//printk("sgs->dma_lenght = %d\n", sgs->dma_length); //5.17
		/*
		 * NOTE: the dam_lenght cannot larger than 1MB now
		 *       should modify here later if the size is lager than 1MB.
		 */
	
		hw->ctl = 0;

		hw->src_addr = sg_dma_address(sgs);
		hw->dst_addr = sg_dma_address(sgd);

		dump_desc_dbg(ioat_chan, desc);

		//desc->txd.flags = flags;
		//desc->len = 4096; //page size, bad... should change
	...
	//printk("ioat_dma_prep_sg !!!\n");/5.17
	return &desc->txd;
	//printk("*****enter ioat_dma_prep_sg after !!!\n");
	//return NULL;
